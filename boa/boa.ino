...
//  colorWipe(strip.Color(0, 0, 255), 50); // Blue
  rainbow(12);
// twinkleRand(5,strip.Color(255, 255, 255),strip.Color(0, 0, 100), 90);
 // rainbowCycle(20);
}
...
void playTwinkle(int pixel, uint32_t startColor) {
  float brightness = 0.0;
   while (brightness < 1) {
      strip.setPixelColor(pixel, (int) 255 * brightness, 0, 0 );
      brightness += .05;
      strip.show();
      delay(20);
   }
...
//	 stripSet(bg,0);
	 ...
	 // for each num
	   //strip.setPixelColor(random(strip.numPixels()),c);
            playTwinkle(random(strip.numPixels()), c);
	//strip.show();
	delay(wait);
}
