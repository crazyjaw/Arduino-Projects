...
void setup() {
  ...
  LEDS.addLeds<WS2812B, DATA_PIN2, RGB>(leds, NUM_LEDS_PER_STRIP).setCorrection( TypicalLEDStrip).setDither(true);
  LEDS.addLeds<WS2812B, DATA_PIN, RGB>(leds, NUM_LEDS_PER_STRIP, NUM_LEDS_PER_STRIP).setCorrection( TypicalLEDStrip).setDither(true);
  delay(50);
  LEDS.setBrightness(200);
  LEDS.setMaxPowerInMilliWatts(4000);
...
void meteorRain( byte meteorSize, byte meteorTrailDecay, boolean meteorRandomDecay, int SpeedDelay) {
  ...
  if (currentTick - lastBeat >= beatInterval  ) {
    lastBeat = millis();
    uint8_t meteorHue = random(256);
    for (int i = 0; i < NUM_LEDS ; i++) {


      // fade brightness all LEDs one step
      for (int j = 0; j < NUM_LEDS; j++) {
        if ( (random(meteorSize) > 6) ) {
...
void pride()
{
  ...
  uint16_t hueinc16 = beatsin88(113, 1, 1500);
  ...
  for ( uint16_t i = 0 ; i < NUM_LEDS; i++) {
    ...
    uint16_t pixelnumber = reverseMap(i);
    pixelnumber = (NUM_LEDS - 1) - pixelnumber;

    if (mode == 7)
      nblend( leds[reverseMap(pixelnumber)], newcolor, 64);
